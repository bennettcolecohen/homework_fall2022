...
class RL_Trainer(object):
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            # Here, we will get the EvalReturn every 1000 training steps
            # if train_step % 1000 == 0:
            #     self.perform_intra_episode_logging(train_step, self.agent.actor)
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)): 
            new_label = expert_policy.get_action(paths[i]['observation'])
            paths[i]['action'] = new_label
    ...
    def perform_intra_episode_logging(self, iter, eval_policy):        
