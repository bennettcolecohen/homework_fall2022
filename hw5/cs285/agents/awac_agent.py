...
class AWACAgent(DQNAgent):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        ob_no, ac_na = ptu.from_numpy(ob_no), ptu.from_numpy(ac_na)
        ...
        if self.agent_params['discrete']:
            ...
            for i in range(self.agent_params['ac_dim']):
                actions = i * torch.ones(batch_size)
                log_pi_vals = torch.exp(dist.log_prob(actions))
                q_val = self.get_qvals(self.exploitation_critic, ob_no, actions) * log_pi_vals
                vals.append(q_val)
        else:
            
            for _ in range(n_actions):
                acs = self.actor.get_action(ob_no)
                q_val = self.get_qvals(self.exploitation_critic, ob_no, acs)
                vals.append(q_val)
        
        v_pi = torch.stack(vals).mean(dim = 0)

        # TODO Calculate Q-Values
        q_vals = q_vals = self.get_qvals(self.exploitation_critic, ob_no, ac_na)
